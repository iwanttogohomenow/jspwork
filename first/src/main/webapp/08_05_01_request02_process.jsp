...
<body>
	...
	아이디:
	<jsp:getProperty name="memberBean" property="id" /><br> 
	<!-- memberBean.getId(); -->
	이름:
	<jsp:getProperty name="memberBean" property="name" /><br> 
	<!-- memberBean.getName(); -->
	취미:
	<jsp:getProperty name="memberBean" property="hobby" /><br> 
	memberBean.getHobby();
	요청정보 컨텐츠타입 =
	<%= request.getContentType() %><br>
	<small style="color: red">클라이언트가 전송한 요청 정보(post로 보낸 데이터만)와 컨텐츠
		타입을 구함. String으로 반환</small>
	<br>
...
for(int i= 0; i < hobby.length; i++){
	if(hobby[i] != null) {
		...
		if(i ==0) out.write("입력한 취미 정보가 확인되지 않습니다.");
	}
	}
